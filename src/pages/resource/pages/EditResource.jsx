...
export default class EditResource extends React.Component {
    ...
    componentDidMount() {
        ...
        let item = this.props.location.state.resource
        if (item) {
            // console.log(item);
            this.resourceRef.current.setFieldsValue(item)

            this.setState({
                imageUrl: item.resource_img, // 资源封面
                focusImgUlr: item.focus_img, // 轮播图封面
                resource_id: item.id,
                resource_content_tag: item.resource_content
            });
    ...
    render() {
        ...
        const onFinish = values => {
            let { resource_id, imageUrl, focusImgUlr, dragFileList, resource_content_tag } = this.state
            if (!imageUrl) {
            ...
            editResource(getUser().token, resource_id, values.resource_name,
                values.resource_author, resource_publish_time, dragFileList,
                values.resource_category_id, values.resource_classes_id,
                values.resource_area_id, values.resource_meta_id, values.resource_format_id,
                imageUrl, values.resource_price, focusImgUlr, resource_content_tag)
                .then((result) => {
                    console.log(result);
        ...
        return (
            <Card title="编辑新增幼教资源">
                <Form  {...formItemLayout} ref={this.resourceRef} onFinish={onFinish}>
                    <Form.Item
                        label={"资源名称"}
                        name="resource_name"
