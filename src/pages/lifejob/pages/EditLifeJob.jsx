...
import { editJob,getJobPre,getJobFamily } from "../../../api/lifejobApi"
...
export default class AddLift extends Component {
    ...
    componentDidMount() {
        ...
        console.log(item);
        if(item){
            let itemjob = this.props.location.state.lifejob
            console.log(itemjob);
            if(itemjob){
                this.fromRef.current.setFieldsValue(itemjob)
                this.setState({
                    job_id:itemjob.id,
                    imageUrl:itemjob.job_img,
                    focusImgUlr:itemjob.focus_img,
                    job_content:itemjob.job_content,
                })
            }
    ...
    render() {
        ...
        let {job_id,focusImgUlr, imageUrl,job_content, job_pre_edu, job_famil } = this.state
        const onFinish = values => {
            ...
            editJob(getUser().token, job_id,values.job_name, imageUrl,values.job_author, job_time,job_content, values.job_pre_edu_id,  values.job_family_edu_id,focusImgUlr)
            .then(result=>{
                console.log(result);
                if(result.status===1){
        ...
        return (
            <Card title="修改新增人生资源">
                <Form  {...formItemLayout} onFinish={onFinish} ref={this.fromRef}>
                    <Form.Item
                        label={"人生名称"}
                        name="job_name"
